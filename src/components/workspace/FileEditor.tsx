 ...
 * Supports:
 * - Code files (with CodeMirror 6)
 * - Markdown files (with preview)
 * - Images
 * - Binary files (warning)
 * - Large files (>1MB) with optimization
 */
...
export function FileEditor({
  ...
  // Determine file type
  const isImage = ['png', 'jpg', 'jpeg', 'gif', 'svg', 'bmp', 'webp'].includes(fileType);
    ...
    <div className="flex-1 flex flex-col min-w-0 h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <FileIcon type={metadata?.filename || ''} size={20} />
...
function FileIcon({ type, size = 20 }: FileIconProps) {
